...
// Twilio posts one status per message per transition to the statusCallback
// URL. They can arrive out of order (a 'sent' after a 'delivered' is
// common), and the same status can arrive twice on a retry. This decides
// what, if anything, the callback should write for a row whose current
// delivery_status is `current`. Pure; the webhook does the I/O.
//
// Returns null for "write nothing", else { deliveryStatus, delivered, failed }.
var SMS_STATUS_RANK = { queued: 1, accepted: 1, scheduled: 1, sending: 2, sent: 3, delivered: 4, read: 5, undelivered: 4, failed: 4, canceled: 4 };
var SMS_TERMINAL_FAILURES = ['undelivered', 'failed', 'canceled'];
function smsStatusTransition(current, incoming) {
  var next = String(incoming || '').toLowerCase();
  if (!SMS_STATUS_RANK[next]) return null;              // unknown status: ignore
  var cur = String(current || '').toLowerCase();
  if (cur === next) return null;                        // duplicate callback
  if (SMS_STATUS_RANK[cur] && SMS_STATUS_RANK[cur] > SMS_STATUS_RANK[next]) return null; // late, older
  if (SMS_STATUS_RANK[cur] === SMS_STATUS_RANK[next] && cur) return null; // e.g. delivered vs failed at equal rank: keep first word
  return {
    deliveryStatus: next,
    delivered: next === 'delivered' || next === 'read',
    failed: SMS_TERMINAL_FAILURES.indexOf(next) >= 0
  };
}

module.exports = {
  smsStatusTransition: smsStatusTransition,
  SMS_TERMINAL_FAILURES: SMS_TERMINAL_FAILURES,
