...
// The UTC instant at which the calendar day `localDay` (YYYY-MM-DD) ENDS in
// `tz` — i.e. the first moment of the next local day, as epoch seconds.
// Used as the APNs expiry on a NO_TEST push (§4.12a): a clearance is a
// statement about today and must not arrive tomorrow.
//
// Built by scanning rather than by offset arithmetic so a DST change on the
// day in question cannot produce a 23- or 25-hour "day" by accident: start
// from the day's noon UTC, step forward an hour at a time until the wall
// clock in `tz` reads the NEXT day, then step back a minute at a time until
// it reads `localDay` again, and add that one minute. At most ~36 hourly
// steps and ~60 minute steps — trivial for a per-push call.
function endOfLocalDayEpochSeconds(localDay, tz) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(localDay || ''))) return null;
  var t = Date.parse(localDay + 'T12:00:00Z');
  if (isNaN(t)) return null;
  var guard = 0;
  while (formatLocalDay(new Date(t), tz) <= localDay && guard++ < 48) t += 3600 * 1000;
  guard = 0;
  while (formatLocalDay(new Date(t - 60 * 1000), tz) > localDay && guard++ < 120) t -= 60 * 1000;
...
module.exports = {
  ...
  todayMD: todayMD,
  endOfLocalDayEpochSeconds: endOfLocalDayEpochSeconds,
