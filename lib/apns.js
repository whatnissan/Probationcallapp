...
function buildPayload(opts) {
  return {
    aps: {
      ...
      sound: 'default',
      // Time Sensitive — see the header comment above. OVERRIDABLE, and the
      // only caller that overrides it is the admin push test: breaking
      // through Focus is justified for a 5 AM MUST_TEST and is not
      // justified for a diagnostic. Defaults are unchanged, so every real
      // notification keeps the behaviour it had.
      'interruption-level': opts.interruptionLevel || 'time-sensitive',
      'relevance-score': typeof opts.relevanceScore === 'number'
        ? opts.relevanceScore
        : (opts.mustTest ? 1 : 0.5)
    },
