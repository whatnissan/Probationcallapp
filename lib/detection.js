...
const FTBEND_COLORS = [
  ...
  'tan', 'teal', 'tin', 'turquoise', 'violet', 'white', 'wine', 'yellow'
];

// Phase group strings (Rosenberg 2 announces these alongside colors)
// Generated rather than hand-listed. The hand-written list stopped at
// 'phase 4', so the 2026-08-04 announcement "phase one b and phase five"
// lost Phase 5 entirely: detectColor returned null for that part and
// doCrossCheck could not substring-match "Phase 5" against "phase five".
// Nine retries against a byte-identical transcript followed. Over-generating
// is safe — validateFtbendColor only ACCEPTS from this list, so an unused
// entry can never invent an answer.
const FTBEND_PHASES = (function () {
  var out = ['prep'];
  for (var n = 1; n <= 10; n++) {
    out.push('phase ' + n);
    ['a', 'b', 'c'].forEach(function (suffix) { out.push('phase ' + n + ' ' + suffix); });
  }
  return out;
})();
...
// Normalize spelled-out phase numerals to digits ("phase one" -> "phase 1")
// so they match the canonical FTBEND_PHASES entries. Used by BOTH detectColor
// (Pass 1 word-boundary match) AND doCrossCheck (substring match against
// ground truth like "Phase 1"). doCrossCheck previously matched ground truth
// against the RAW transcript — so a "Phase 1" ground truth never substring-
// matched a "phase one" transcript, producing a permanent no_match retry loop
// (2026-06-11 Rosenberg 2: 19+ attempts, never resolved). The word-boundary
// anchor keeps this from touching unrelated "one" mentions (e.g. "press one").
var PHASE_WORD_NUMERALS = {
  one: 1, two: 2, three: 3, four: 4, five: 5,
  six: 6, seven: 7, eight: 8, nine: 9, ten: 10
};

function normalizePhaseNumerals(s) {
  var out = String(s || '').toLowerCase();
  // Spelled-out numerals. Previously stopped at four, which is exactly how
  // "phase five" survived to the matcher unrecognised.
  Object.keys(PHASE_WORD_NUMERALS).forEach(function (word) {
    out = out.replace(new RegExp('\\bphase\\s+' + word + '\\b', 'g'), 'phase ' + PHASE_WORD_NUMERALS[word]);
  });
  // Spacing variants: "phase 1a" / "phase1b" / "phase 1 a" all become the
  // canonical "phase 1 a", so one vocabulary entry covers how the hotline
  // and the transcriber happen to space it on any given day.
  out = out.replace(/\bphase\s*(\d{1,2})\s*([abc])\b/g, 'phase $1 $2');
  out = out.replace(/\bphase\s*(\d{1,2})\b/g, 'phase $1');
  return out;
}
...
function detectColor(transcript) {
  ...
  var lower = normalizePhaseNumerals(transcript);
...
function detectPhaseColors(transcript) {
  // Normalise BEFORE the anchor/split so word-numerals and spacing variants
  // are canonical by the time each part reaches detectColor.
  var lower = normalizePhaseNumerals(transcript);
...
function doCrossCheck(transcript, ourDetection, groundTruthArr) {
  ...
  // Normalize spelled-out phase numerals so a "Phase 1" ground-truth item
  // substring-matches a "phase one" transcript. Without this, multi-word
  // phase ground truth never resolved and looped on no_match forever.
  var transcriptLower = normalizePhaseNumerals(transcript);
  ...
  if (!allResolved) {
    // Distinguish "we got some of it" from "we got none of it". Previously
    // both scored no_match, so a detection that was half right — Phase 1 B
    // correct, Phase 5 dropped — looked identical to complete failure, and
    // the retry loop had no way to tell a parser gap from a transient one.
    var resolvedCount = groundTruthArr.filter(function(item) {
      var il = item.toLowerCase();
      if (transcriptLower.indexOf(il) !== -1) return true;
      if (il.indexOf(' ') !== -1) return false;
      return transcriptTokens.some(function(t) { return phoneticMatch(t, il); });
    }).length;
    return {
      match_method: resolvedCount > 0 ? 'partial_match' : 'no_match',
      final_answer: null,
      misrecognition_added: null,
      resolved_count: resolvedCount,
      expected_count: groundTruthArr.length
    };
  }
...
module.exports = {
  ...
  normalizePhaseNumerals: normalizePhaseNumerals,
